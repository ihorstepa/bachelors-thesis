async function Module(moduleArg = {}) {
    ...
    function findWasmBinary() {
        if (Module['locateFile']) {
            return locateFile('clangd.wasm')
        }
        return new URL('clangd.wasm', import.meta.url).href
    }
    ...
    var PThread = {
        ...
        allocateUnusedWorker() {
            ...
            if (Module['mainScriptUrlOrBlob']) {
                ...
                worker = new Worker(pthreadMainJs, { type: 'module', name: 'em-pthread-' + PThread.nextWorkerID })
            } else
                worker = new Worker(new URL('clangd.js', import.meta.url), {
                    type: 'module',
    ...
    function ___syscall_statfs64(path, size, buf) {
        if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, path, size, buf)
        path >>>= 0
        size >>>= 0
        buf >>>= 0
        try {
    ...
    function ___syscall_unlinkat(dirfd, path, flags) {
        if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, dirfd, path, flags)
        path >>>= 0
        try {
            path = SYSCALLS.getStr(path)
            path = SYSCALLS.calculateAt(dirfd, path)
    ...
    function __emscripten_fs_load_embedded_files(ptr) {
        ...
        do {
            ...
            FS.createDataFile(
                name,
                null,
                (growMemViews(), HEAP8).subarray(content >>> 0, (content + len) >>> 0),
                true,
                true,
                true,
            )
    ...
    function __munmap_js(addr, len, prot, flags, fd, offset) {
        if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, addr, len, prot, flags, fd, offset)
        addr >>>= 0
        len >>>= 0
        offset = bigintToI53Checked(offset)
        try {
    ...
    function __setitimer_js(which, timeout_ms) {
        if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 1, which, timeout_ms)
        if (timers[which]) {
    ...
    function _fd_fdstat_get(fd, pbuf) {
        if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 0, 1, fd, pbuf)
        pbuf >>>= 0
        try {
            var rightsBase = 0
    ...
    function __asyncjs__waitForStdin() {
    ...
    var dynCall_viiiji = makeInvalidEarlyAccess('dynCall_viiiji')
    ...
    var dynCall_vijjjjjii = makeInvalidEarlyAccess('dynCall_vijjjjjii')
    ...
    function assignWasmExports(wasmExports) {
        ...
        assert(typeof wasmExports['dynCall_viiiij'] != 'undefined', 'missing Wasm export: dynCall_viiiij')
        ...
        assert(typeof wasmExports['dynCall_vijjjjjii'] != 'undefined', 'missing Wasm export: dynCall_vijjjjjii')
        ...
        dynCall_viiiij = dynCalls['viiiij'] = createExportWrapper('dynCall_viiiij', 6)
        ...
        dynCall_vijjjjjii = dynCalls['vijjjjjii'] = createExportWrapper('dynCall_vijjjjjii', 9)
