...
if (
    elemToggleShowSuggestions == null ||
    elemToggleSuggestMode == null ||
    elemToggleConnect == null ||
    elemToggleRenderVersion == null ||
    elemRollbackBtn == null
)
...
const providerYdoc = new WebsocketProvider(yhubUrl, docid, ydoc, {
...
const initEditorBindingSuggestions = () => {
    ...
    const docContent = ytext
        .toDelta(attributionManager)
        .children.map(
            s
                .match()
                .if(delta.$textOp, (op) => op.insert)
                .else(() => '')
                .done(),
        )
        .join('')
    const state = EditorState.create({
        ...
        extensions: [
            ...
            yCollab(ytext, (withSuggestions ? providerYdocSuggestions : providerYdoc).awareness, {
                attributionManager,
            }),
            // oneDark
        ],
...
const initEditorBindingVersionDiff = (prevDoc, nextDoc, attributions) => {
    const diffAttributions = Y.createAttributionManagerFromDiff(prevDoc, nextDoc, {
        attrs: attributions,
    })
    const ytext = nextDoc.get('quill')
    const docContent = ytext
        .toDelta(diffAttributions)
        .children.map(
            s
                .match()
                .if(delta.$textOp, (op) => op.insert)
...
const renderVersions = async (from, to) => {
    try {
        const response = await fetch(
            `${yhubApiUrl}/changeset/${org}/${docid}?yauth=${authToken}&from=${from}&to=${to}&ydoc=true&attributions=true`,
        )
        if (response.ok) {
...
const rollback = async () => {
    ...
    try {
        const response = await fetch(`${yhubApiUrl}/rollback/${org}/${docid}?yauth=${authToken}`, {
            ...
            body: Buffer.from(
                buffer.encodeAny({
                    from: currentVersionRange.from,
                    to: currentVersionRange.to,
                }),
            ),
        })
        if (response.ok) {
