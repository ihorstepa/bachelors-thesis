    ...
    if (from || to || by || (customAttributions != null && customAttributions.length > 0)) {
        ...
        const attrFilter = (attrs) => {
            if (
                ...
                !attrs.some(
                    (attr) =>
                        (attr.name === 'insertAt' || attr.name === 'deleteAt') &&
                        (from == null || attr.val >= from) &&
                        (to == null || attr.val <= to),
                )
            ) {
                return false
            ...
            if (
                ...
                !attrs.some(
                    (attr) =>
                        (attr.name === 'insert' || attr.name === 'delete') &&
                        /** @type {string} */ (attr.val).split(',').includes(by),
                )
            ) {
                return false
            }
            if (
                customAttributions != null &&
                !customAttributions.every((customAttr) =>
                    attrs.some((attr) => {
...
port.on(
    'message',
    /** @param {import('./compute.js').ComputeTask} msg */ (msg) => {
        ...
        switch (msg.type) {
            case 'mergeUpdatesAndGc': {
                ...
                ydoc.transact(() => {
                    msg.updates.forEach(
                        /** @param {Uint8Array} update */ (update) => {
            ...
            case 'changeset': {
                ...
                const filteredAttributions = filterContentMap(
                    contentmap,
                    from ?? undefined,
                    to ?? undefined,
                    by || undefined,
                ...
                if (includeYdoc || includeDelta) {
                    ...
                    if (includeDelta) {
                        ...
                        const am = Y.createAttributionManagerFromDiff(prevDoc, nextDoc, {
                            attrs: filteredAttributions,
                        })
            ...
            case 'activity': {
                ...
                activity.forEach((act) => {
                    if (
                        lastActivity != null &&
                        lastActivity.by === act.by &&
                        act.from - lastActivity.to < groupDistance
                    ) {
                ...
                if (includeDelta) {
                    activityResult.forEach((act) => {
                        const actAttributions = filterContentMap(
                        ...
                        const beforeContentIds = Y.createContentIdsFromContentMap(
                            filterContentMap(
                                contentmap,
                                0,
                                act.from != null ? act.from - 1 : undefined,
                                undefined,
                                undefined,
                                null,
                            ),
                        ...
                        Y.applyUpdate(prevDoc, prevDocUpdate)
                        Y.applyUpdate(nextDoc, nextDocUpdate)
                        const attrs = Y.createContentMapFromContentIds(
                            Y.createContentIdsFromContentMap(actAttributions),
                            [
                                Y.createContentAttribute('insert', act.by),
                                Y.createContentAttribute('insertAt', act.from),
                            ],
                            [
                                Y.createContentAttribute('delete', act.by),
                                Y.createContentAttribute('deleteAt', act.from),
                            ],
                        )
                        const am = Y.createAttributionManagerFromDiff(prevDoc, nextDoc, {
                            attrs,
                        ...
                        act.delta = nextDoc
                            .get(nextDoc.share.keys().next().value || '')
                            .toDeltaDeep(am)
                            .toJSON()
                        prevDoc.destroy()
                        nextDoc.destroy()
            ...
            case 'rollback': {
                ...
                const contentmap = Y.decodeContentMap(contentmapBin)
                const contentIds = contentIdsBin && Y.decodeContentIds(contentIdsBin)
                const reducedAttributions = filterContentMap(
                    contentmap,
                    from,
                    to,
                    by,
                ...
                ydoc.once('update', (/** @type {Uint8Array<ArrayBuffer>} */ u) => {
                    update = u
                })
                Y.undoContentIds(ydoc, revertIds, { ignoreRemoteMapChanges: true })
                ydoc.destroy()
                if (update != null) {
                    const resultContentmap = createContentMap(
                        Y.createContentIdsFromUpdate(update),
                        userid,
                        customAttributions,
                    )
