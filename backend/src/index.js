...
export class YHub {
                ...
                await promise.all(
                    tasks.map(async (task) => {
                        ...
                        if (task.type === 'compact') {
                            ...
                            const d = await this.getDoc(task.room, {
                                gc: true,
                                nongc: true,
                                contentmap: true,
                                contentids: true,
                            ...
                            if (!strm.isSmallerRedisClock(d.lastPersistedClock, d.lastClock)) {
                                ...
                                await this.stream.trimMessages(
                                    task.room,
                                    d.lastClock,
                                    this.stream.minMessageLifetime,
                                    task.redisClock,
                                )
                            ...
                            taskLog.info(
                                {
                                    gcDocSize: d.gcDoc?.byteLength,
                                    nongcDocSize: d.nongcDoc?.byteLength,
                                    refsDeleted: d.references?.length ?? 0,
                                },
                                'task completed',
                            )
                        }
    ...
    async getDoc(room, includeContent, { gcOnMerge = true } = {}) {
        const [persistedDoc, cachedMessages] = await promise.all([
            this.persistence.retrieveDoc(
                room,
                object.assign({}, includeContent, {
        ...
        return {
            gcDoc: /** @type {Include['gc'] extends true ? Uint8Array<ArrayBuffer> : null} */ (
                gcDoc
                    ? gcOnMerge
                        ? await this.computePool.mergeUpdatesAndGc(gcDoc)
