...
export const createYHubServer = async (yhub, conf) => {
        ...
        const handleRollbackRequest = async (buffer) => {
            ...
            try {
                ...
                if (
                    ...
                    const { contentmap: contentmapBin, nongcDoc } = await yhub.getDoc(room, {
                        nongc: true,
                        contentmap: true,
                    })
    ...
    app.get('/changeset/:org/:docid', async (res, req) => {
        ...
        const withCustomAttributions = withCustomAttributionsParam
            ? parseCustomAttributionsParam(withCustomAttributionsParam)
            : null
        let aborted = false
        res.onAborted(() => {
            aborted = true
        ...
        const cacheArgs = [
            ...
            String(includeYdoc),
            String(includeDelta),
            String(includeAttributions),
            withCustomAttributionsParam || '',
        ]
        const responseData = await yhub.stream.cachedGet('changeset', cacheArgs, async () => {
            const { nongcDoc, contentmap: contentmapBin } = await yhub.getDoc(room, {
                nongc: true,
                contentmap: true,
            })
    ...
    app.get('/activity/:org/:docid', async (res, req) => {
        ...
        const cacheArgs = [
            ...
            String(group),
            withCustomAttributionsParam || '',
            String(includeCustomAttributions),
            contentIdsParam || '',
        ]
