...
const getFreeWorker = (pool) => {
    ...
    for (let i = 0; i < pool.workers.length; i++) {
        ...
        if (w.isComputing && now - w.taskStart > maxTaskDurationMs) {
            log.warn(
                { workerIndex: i, taskDurationMs: now - w.taskStart },
                'terminating worker that exceeded max task duration',
            )
            w.terminate()
        }
        if (w.isDead) {
...
const drain = (pool) => {
    while (pool.queue.length > 0) {
        ...
        if (!worker) break
        const task =
            /** @type {{ task: ComputeTask, transferList: ArrayBuffer[], resolve: (value: any) => void, reject: (reason: any) => void }} */ (
                pool.queue.shift()
            )
