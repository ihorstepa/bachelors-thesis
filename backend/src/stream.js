 ...
 * @param {string} prefix
 */
export const encodeRoomName = (room, prefix) =>
    `${prefix}:room:${encodeURIComponent(room.org)}:${encodeURIComponent(room.docid)}:${encodeURIComponent(room.branch)}`
...
export const decodeRoomName = (rediskey, expectedPrefix) => {
    ...
    return {
        ...
        branch: decodeURIComponent(match[4]),
    }
}
...
export class Stream {
        ...
        if (!this._subRunning) {
            ...
            while (this.subs.size > 0 || this.subUpdates.size > 0) {
                ...
                this.subUpdates.forEach((update, streamName) => {
                    const s = map.setIfUndefined(this.subs, streamName, () => ({
                        lastReceivedClock: update.lastReceivedClock,
                        subs: /** @type {Set<StreamSubscriber>} */ (new Set()),
                    }))
                ...
                try {
                    ...
                    for (let i = 0; i < ms.length; i++) {
                        ...
                        if (sub != null) {
                            sub.subs.forEach((s) => {
                                const filteredMessages = m.messages.filter((m) =>
                                    isSmallerRedisClock(s.lastReceivedClock, m.redisClock),
                                )
                    ...
                    ms.length > 0 &&
                        log.debug(
                            { messageCount: ms.length, subscriberCount: nsubCounter },
                            'pulled messages and notified subscribers',
                        )
                } catch (e) {
    ...
    async getMessages(rooms, { redisClient = this.redis, blocking = false } = {}) {
        ...
        const streams = rooms.map((asset) => ({
            key: s.$string.check(asset.room) ? asset.room : encodeRoomName(asset.room, this.prefix),
            id: asset.clock || '0',
        }))
        log.debug({ streamCount: streams.length }, 'retrieving messages')
        const reads =
            /** @type {Array<{name: Buffer, messages: Array<{id: Buffer, message: Record<string, Buffer>}>}> | null} */ (
                await redisClient
                    .withTypeMapping({
                        [redis.RESP_TYPES.BLOB_STRING]: Buffer,
                    })
                    .xRead(streams, blocking ? { BLOCK: 200, COUNT: 5000 } : {})
            )
        /**
    ...
    subscribe(room, subscriber) {
        ...
        const s = map.setIfUndefined(this.subUpdates, streamName, () => ({
            lastReceivedClock: subscriber.lastReceivedClock,
            subs: /** @type {Set<StreamSubscriber>} */ (new Set()),
        }))
    ...
    async claimTasks(count) {
        ...
        const tasks = reclaimedTasks.messages
            .map((m) => {
    ...
    async cachedGet(endpoint, args, computeResult) {
        ...
        const cached = await /** @type {Promise<Buffer | null>} */ (
            this.redis
                .withTypeMapping({
                    [redis.RESP_TYPES.BLOB_STRING]: Buffer,
                })
        ...
        if (result.byteLength < 100 * 1000 * 1000) {
            ...
            this.redis
                .set(key, Buffer.from(result), { EX: this.cacheTtl + computeTime * 2 })
                .catch((err) => log.error({ err }, 'error caching result'))
        }
        return result
    }
}
...
export const createStream = async (config) => {
    ...
    try {
        await ystream.redis.xGroupCreate(ystream.workerStreamName, ystream.workerGroupName, '0', {
            MKSTREAM: true,
        })
    } catch (e) {
