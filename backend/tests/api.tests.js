...
export const testChangesetRestApi = async (tc) => {
    ...
    {
        const changeset = await fetchYhubResponse(
            `/changeset/${org}/${ydoc.guid}?from=${activity[1].from}&to=${activity[1].to}&ydoc=true&delta=true&attributions=true`,
        )
    ...
    {
        // rollback
        const rollbackResult = await postYhubRequest(`/rollback/${org}/${ydoc.guid}`, {
            from: activity[1].from,
            to: activity[1].to,
        }) // undo (delete "hello" & insert "!")
...
export const testWorker = async (tc) => {
    ...
    const { ydoc, provider } = await createWsClient({
        waitForSync: true,
        syncAwareness: false,
    })
    ydoc.get().setAttr('key1', 'val1')
    ...
    await promise.untilAsync(
        async () => (await yhub.stream.redis.exists(streamName)) === 1,
        yhub.conf.redis.minMessageLifetime * 9,
    )
    t.info('stream deleted')
    const { ydoc: loadedDoc } = await createWsClient({
        waitForSync: true,
        syncAwareness: false,
    })
...
export const testYdocRestApi = async (tc) => {
    ...
    t.compare(
        localDoc.get().toDelta(),
        delta.create().insert('initial content').done(),
        'Local doc should have initial content',
    )
    ...
    const { ydoc: verifyDoc } = await createWsClient({
        docid: initialDoc.guid.split('-').pop(),
        waitForSync: true,
    })
    t.compare(
        verifyDoc.get().toDelta(),
        delta.create(delta.$deltaAny).insert('initial new content'),
...
export const testCustomAttributionsRollback = async (tc) => {
     ...
     * @type {Array<any>}
     */
    const activityUserA = await fetchYhubResponse(
        `/activity/${org}/${initialDoc.guid}?group=false&withCustomAttributions=source:userA&delta=true&customAttributions=true&delta=true`,
    )
    ...
    try {
        for (let currIteration = 0; currIteration < Iterations; currIteration++) {
            ...
            await t.measureTimeAsync(`time to sync ${NDocs} docs`, async () => {
                await promise.all(
                    array.unfold(NDocs, async (i) => {
                        const r = await createWsClient({
                            waitForSync: true,
                            syncAwareness: false,
                            docid: 'doc-' + i,
                        })
        ...
        console.log({
            maxMemory: maxMemory / 1000 / 1000,
            beforeMemory: beforeMemory / 1000 / 1000,
            cleanedUpMemory: cleanedUpMemory / 1000 / 1000,
            diff: (cleanedUpMemory - beforeMemory) / 1000 / 1000,
        })
...
export const testActivityContentIdsFilter = async (tc) => {
    ...
    const getAttributeHistory = async (ytype, key) => {
        ...
        const contentIdsParam = encodeURIComponent(
            buffer.toBase64(Y.encodeContentIds(Y.createContentIds(idset, idset))),
        )
        return await fetchYhubResponse(
            `/activity/${org}/${ydoc.guid}?group=false&contentIds=${contentIdsParam}&delta=true`,
        )
    }
