import * as Y from '@y/y'
import * as t from 'lib0/testing'
import { defaultOrg, yhub } from './utils.js'
import { HISTORY_FEATURE_ENABLED } from '../src/config.js'

let currClock = 0

/**
 * @param {string} org
 * @param {string} docid
 * @param {Y.Doc} ydoc
 */
const storeDoc = (org, docid, ydoc) => {
    const encDoc = Y.encodeStateAsUpdate(ydoc)
    const contentids = Y.createContentIdsFromDoc(ydoc)
    return yhub.persistence.store(
        { org, docid, branch: 'main' },
        {
            lastClock: ++currClock + '',
            gcDoc: encDoc,
            nongcDoc: encDoc,
            contentids: Y.encodeContentIds(contentids),
            contentmap: Y.encodeContentMap(Y.createContentMapFromContentIds(contentids, [], [])),
        },
    )
}

/**
 * @param {string} org
 * @param {string} docid
 */
const retrieveDoc = async (org, docid) => {
    const { gcDoc: ydocBin, nongcDoc: nongcBin } = await yhub.getDoc(
        { org, docid, branch: 'main' },
        { gc: true, nongc: true },
        { gcOnMerge: false },
    )
    return {
        ydoc: Y.createDocFromUpdate(ydocBin),
        nongcYdoc: nongcBin ? Y.createDocFromUpdate(nongcBin) : null,
    }
}

/**
 * @param {t.TestCase} tc
 */
export const testUnsafePersistDoc = async (tc) => {
    const org = defaultOrg
    const room = { org, docid: `${tc.testName}-index`, branch: 'main' }

    t.info('persisting two docs via unsafePersistDoc')
    const ydoc1 = new Y.Doc()
    ydoc1.get().setAttr('a', 1)
    await yhub.unsafePersistDoc(room, Y.encodeStateAsUpdate(ydoc1), {
        by: 'alice',
    })

    const ydoc2 = new Y.Doc()
    ydoc2.get().setAttr('b', 2)
    await yhub.unsafePersistDoc(room, Y.encodeStateAsUpdate(ydoc2), {
        by: 'bob',
    })

    t.info('retrieving and asserting merged content')
    const { gcDoc: ydocBin } = await yhub.getDoc(room, { gc: true }, { gcOnMerge: false })
    const merged = Y.createDocFromUpdate(ydocBin)
    t.assert(merged.get().getAttr('a') === 1)
    t.assert(merged.get().getAttr('b') === 2)
}

/**
 * @param {t.TestCase} tc
 */
export const testStorage = async (tc) => {
    const org = defaultOrg
    const indexDocid = `${tc.testName}-index`
    const doc3Docid = `${tc.testName}-doc3`
    {
        t.info('persisting docs')
        // index doc for baseline
        const ydoc1 = new Y.Doc()
        ydoc1.get().setAttr('a', 1)
        await storeDoc(org, indexDocid, ydoc1)
        // second doc with different changes under the same index key
        const ydoc2 = new Y.Doc()
        ydoc2.get().setAttr('b', 1)
        await storeDoc(org, indexDocid, ydoc2)
        // third doc that will be stored under a different key
        const ydoc3 = new Y.Doc()
        ydoc3.get().setAttr('a', 2)
        await storeDoc(org, doc3Docid, ydoc3)
    }
    {
        t.info('retrieving docs')
        const r1 = await retrieveDoc(org, indexDocid)
        const doc1 = r1.ydoc
        // should have merged both changes..
        t.assert(doc1.get().getAttr('a') === 1 && doc1.get().getAttr('b') === 1)
        const nongcA = r1.nongcYdoc?.get().getAttr('a')
        const nongcB = r1.nongcYdoc?.get().getAttr('b')
        t.assert(HISTORY_FEATURE_ENABLED ? nongcA === 1 && nongcB === 1 : nongcA === undefined && nongcB === undefined)
        // retrieve other doc..
        const r3 = await retrieveDoc(org, doc3Docid)
        t.assert(r3)
        const doc3 = r3.ydoc
        t.assert(doc3.get().getAttr('a') === 2)
        t.info('delete references')
    }
}
