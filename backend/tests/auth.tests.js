...
const authPrivateKey = await ecdsa.importKeyJwk({
    ...
    ext: true,
    kty: 'EC',
    x: '96pShK8Z3iJ8UZpN4tuyv9CuPuzwWgC_I72N6ZUNWOSBDflVxwYPtL3PcCgCF2aE',
    y: 'Q39u2jtATgoBd9D8Tx744v6KljwE3iOZr30Rf8yuVT3UgGEi0bcKufUGVSeKls8s',
    crv: 'P-384',
    d: 'BS_hqq6UMpuqS10oIWzEyTUt7RRQrysUMUdlwUyVimV_CTTNEpxXFW9_D0NA9rHt',
})
...
export const testSampleAuthServer = async (tc) => {
    ...
    const { ydoc: ydoc1 } = await createWsClient({
    ...
    await t.groupAsync('should not sync if unauthenticated', async () => {
        const { ydoc: ydocUnauthenticated } = createWsClient({
            wsUrl: utils.wsUrlFromPort(authHubPort),
        })
    ...
    await t.groupAsync('should not publish updates from readonly users', async () => {
        ...
        const { ydoc: ydocReadonly } = createWsClient({
            wsUrl: utils.wsUrlFromPort(authHubPort),
